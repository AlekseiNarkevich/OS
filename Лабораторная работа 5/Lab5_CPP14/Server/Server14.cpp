...
string binFl;
int clKol = 0;
unique_ptr<HANDLE[]> allDeadEvents;
unique_ptr<HANDLE[]> YouReadNowEvents;
...
int main()
{
    ...
    cin >> clKol;
    allDeadEvents = make_unique<HANDLE[]>(clKol);
    YouReadNowEvents = make_unique<HANDLE[]>(clKol);
    for (int i = 0; i < clKol; ++i)
    {
        char deadEventName[50];
        sprintf(deadEventName, "DeadEventClient%d", i);
        allDeadEvents[i] = CreateEvent(nullptr, TRUE, FALSE, deadEventName);
        char readReadyEventName[50];
        sprintf(readReadyEventName, "YouCanReadNowClient%d", i);
        YouReadNowEvents[i] = CreateEvent(nullptr, FALSE, FALSE, readReadyEventName);
    }
    ...
    for (int i = 0; i < clKol; ++i)
    {
        ...
        if (!CreateProcess(nullptr, commandLine, nullptr, nullptr, FALSE,
            CREATE_NEW_CONSOLE, nullptr, nullptr, &clSi[i], &clPi[i])) 
        {
    ...
    for (int i = 0; i < clKol; ++i) 
    {
        ...
        if (ConnectNamedPipe(hPipe, nullptr))
        {
            ...
            auto pArgs = make_unique<ClientThreadArgs>();
            pArgs->hPipe = hPipe;
    ...
    if (clKol > 0)
    {
        WaitForMultipleObjects(clKol, allDeadEvents.get(), TRUE, INFINITE);
        cout << "Все клиенты завершили работу" << endl;
    }
    for (auto hThread : clhThreadH)
    {
...
DWORD WINAPI ClientHandlerThread(LPVOID lpParam)
{
    unique_ptr<ClientThreadArgs> args(static_cast<ClientThreadArgs*>(lpParam));
    ...
    while (clientActive)
    {
        if (!ReadFile(hPipe, &request, sizeof(Message), &bytesRead, nullptr) || bytesRead == 0) 
        {
            clientActive = false;
            continue;
        }
        ...
        if (!WriteFile(hPipe, &response, sizeof(Message), &bytesWritten, nullptr))
        {
